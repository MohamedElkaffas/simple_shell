...
void cd_dot(data_shell *datash)
{
	...
	if (!_strcmp(".", dir))
	{
		set_env("PWD", cp_pwd, datash), free(cp_pwd);
		return;
	}
	if (!_strcmp("/", cp_pwd))
	{
 ...
 * @datash: data relevant (environmental)
...
void cd_to_home(data_shell *datash)
{
	...
	if (chdir(home) == -1)
	{
		get_error(datash, 2), free(p_pwd);
		return;
	}
	set_env("OLDPWD", p_pwd, datash);
	set_env("PWD", home, datash), free(p_pwd);
	datash->status = 0;
}
